...
def test_visual_explorer_defaults_to_full_neighbor_rendering() -> None:
    ...
    assert "The connected-node panel lists the full direct neighborhood for" in content
    assert "function radialPositions(graph)" in content
    assert "direct links" in content
    assert "hiddenDirectNodeIds" in content
    assert 'data-neighbor-node-action="focus"' in content
...
def test_visualization_page_enables_external_terms_by_default() -> None:
    ...
    assert 'data-visual-neighbor-summary' in content
    assert 'data-neighbor-preset="show-all"' in content
    assert 'data-visual-class-filters' in content
