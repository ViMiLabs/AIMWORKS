...
<section class="decode-intro">
  ...
  <p>Original GraphML occurrences and directed dependencies are preserved. Cross-workflow expansion is available only through explicit reviewed anchors; the H2KG TBox and its Explore page are unchanged.</p>
  <div class="decode-metrics">
    <span><strong>{counts.get('workflow_count', 0)}</strong> workflows</span>
...
<section id="decode-root" class="decode-root" data-decode-data="{escape(data_href)}" data-h2kg-explore="{escape(h2kg_explore)}">
  ...
  <div class="decode-workspace">
    <div class="decode-controls">
      <fieldset class="decode-view-switch"><legend>Graph view</legend><label><input name="decode-view" value="semantic" type="radio" checked> Semantic overview</label><label><input name="decode-view" value="source" type="radio"> Preserved source occurrences</label></fieldset>
      <label id="decode-source-control"><input id="decode-show-source" type="checkbox" checked> <span id="decode-source-label">Aggregated source dependencies</span></label>
    ...
    <p class="decode-legend"><span class="legend-material">material</span><span class="legend-component">component</span><span class="legend-data">data</span><span class="legend-device">device</span><span class="legend-process">process / method</span><span class="legend-h2kg">H2KG anchor</span><span class="legend-decode">reviewed DECODE anchor</span><br><span class="legend-source">solid arrow</span> preserved source dependency or its provenance-preserving aggregate <span class="legend-anchor">dashed link</span> occurrence-to-anchor mapping in source view <span class="legend-semantic">colored arrow</span> approved H2KG semantic projection</p>
  </div>
...
(() => {
  ...
  const graph = $('#decode-graph'), details = $('#decode-details'), hover = $('#decode-hovercard');
  const sourceToggle = $('#decode-show-source'), anchorToggle = $('#decode-show-anchors'), semanticToggle = $('#decode-show-semantic'), more = $('#decode-more');
  let data, cy, selectedWorkflow, selectedAnchor, shownWorkflows = new Set(), viewMode = 'semantic';
  ...
  function sourceElements() {
    ...
    if (semanticToggle.checked) data.semantic_edges.filter((edge) => nodes.has(edge.source) && nodes.has(edge.target)).forEach((edge) => output.push({ group:'edges', data:{ id:edge.id, source:edge.source, target:edge.target, kind:'semantic', label:edge.label, sourceEdgeIds:[edge.source_dependency_id] } }));
    return output;
  }
  function semanticElements() {
    const visible = visibleNodeIds(), semanticIds = new Set([...visible].map(overviewId)), output = [];
    [...semanticIds].forEach((id) => { const node = data.overviewNodeById[id]; if (node) output.push({ group:'nodes', data:{ id, label:node.label, kind:node.kind, state:node.state, visual:(node.visual_categories || [])[0] || 'other', occurrenceCount:node.occurrence_count, anchorId:data.anchorById[id] ? id : '' } }); });
    ...
    if (semanticToggle.checked) data.semantic_overview.semantic_edges.forEach((edge) => {
  ...
  function renderGraph() {
    ...
    cy = cytoscape({ container:graph, elements:elements(), style:[
      ...
      { selector:'node[kind = "source-anchor"][state = "approved_h2kg"], node[kind = "semantic-anchor"][state = "approved_h2kg"]', style:{ 'background-color':'#0d7f83', 'border-color':'#07575a', 'color':'#fff' } },
      { selector:'node[kind = "source-anchor"][state = "reviewed_decode"], node[kind = "semantic-anchor"][state = "reviewed_decode"]', style:{ 'background-color':'#bd8529', 'border-color':'#855c12', 'color':'#fff' } },
      { selector:'node[kind = "semantic-unresolved"], node[state = "unresolved"]', style:{ 'background-opacity':.55, 'border-style':'dashed' } },
    ...
    cy.on('dbltap', 'node', (event) => { const item = event.target.data(); const anchorId = item.anchorId || (item.kind === 'source-anchor' ? item.id : data.nodeById[item.id]?.anchor_id); if (anchorId) expandAnchor(anchorId); else if (item.kind === 'semantic-unresolved') showSemanticNodeDetails(item.id); else showOccurrenceDetails(item.id); });
    cy.on('tap', 'node', (event) => { const item = event.target.data(); if (item.kind === 'semantic-anchor' || item.kind === 'semantic-unresolved') showSemanticNodeDetails(item.id); else if (item.kind === 'source-anchor') showAnchorDetails(item.id); else showOccurrenceDetails(item.id); });
    cy.on('mouseover', 'node, edge', (event) => { event.target.addClass('hovered'); showHover(event.target, event.originalEvent); });
    cy.on('mousemove', 'node, edge', (event) => moveHover(event.originalEvent));
    cy.on('mouseout', 'node, edge', (event) => { event.target.removeClass('hovered'); hideHover(); });
  }
  ...
  function expandAnchor(anchorId) { selectedAnchor = anchorId; const next = rankedConnectedWorkflows(anchorId)[0]; if (next) shownWorkflows.add(next); viewMode = 'semantic'; syncViewControls(); renderGraph(); showAnchorDetails(anchorId); updateMoreButton(); }
  function addNextConnectedWorkflow(anchorId) { if (anchorId) expandAnchor(anchorId); }
  function updateMoreButton() { const remaining = selectedAnchor ? rankedConnectedWorkflows(selectedAnchor).length : 0; more.hidden = !remaining; more.textContent = `Add next connected workflow (${remaining} remaining)`; }

  function downloads(workflow) { return `<div class="decode-downloads"><a href="../decode/workflows/${encodeURIComponent(workflow.id)}.json" download>Download normalized workflow JSON</a><a href="../decode/workflows/${encodeURIComponent(workflow.id)}.jsonld" download>Download JSON-LD projection</a><a href="../decode/workflows/${encodeURIComponent(workflow.id)}.ttl" download>Download Turtle projection</a><a href="../decode/decode_mapping_matrix.csv" download>Download mapping matrix</a><a href="../decode/decode_duplicate_occurrence_audit.csv" download>Download duplicate-occurrence audit</a><a href="../decode/decode_validation_report.json" download>Download validation report</a></div>`; }
  function showWorkflowDetails(workflow) { const semanticCount = new Set((data.nodesByWorkflow[workflow.id] || []).map(overviewId)).size; details.innerHTML = `<h2>${esc(workflow.title)}</h2><p><span class="decode-status ${esc(workflow.mapping_status)}">${esc(workflow.mapping_status)}</span> <span class="decode-status">${viewMode === 'semantic' ? 'semantic overview' : 'source preserved'}</span></p><p><strong>Source:</strong> ${esc(workflow.source_filename)}<br><strong>Method family:</strong> ${esc(workflow.method_family)}<br><strong>Topology:</strong> ${workflow.source_node_count} occurrences and ${workflow.source_edge_count} directed dependencies.<br><strong>Semantic overview:</strong> ${semanticCount} canonical nodes.</p><p>${viewMode === 'semantic' ? 'Select a canonical node to inspect its original GraphML occurrences and branch contexts.' : 'The complete preserved source workflow is visible. Double-click an occurrence or anchor to traverse reviewed cross-workflow connections.'}</p>${downloads(workflow)}`; }
  ...
  function showSemanticNodeDetails(id) { const node = data.overviewNodeById[id]; if (!node) return; const visible = visibleNodeIds(), occurrences = node.occurrence_ids.filter((occurrenceId) => visible.has(occurrenceId)); const anchor = data.anchorById[id]; const h2kg = anchor?.state === 'approved_h2kg' ? `<p><a href="${root.dataset.h2kgExplore}?iri=${encodeURIComponent(id)}">Inspect this stable H2KG term in H2KG Explore</a></p>` : ''; const role = node.canonical_role_iri && (!anchor || anchor.state !== 'approved_h2kg') ? `<p><strong>H2KG role:</strong> <a href="${root.dataset.h2kgExplore}?iri=${encodeURIComponent(node.canonical_role_iri)}">${esc(node.semantic_role || node.canonical_role_iri)}</a></p>` : ''; const list = occurrences.map((occurrenceId) => { const occurrence = data.nodeById[occurrenceId], workflow = data.workflowById[occurrence.workflow_id]; return `<li><button class="decode-add-workflow" data-inspect-occurrence="${esc(occurrenceId)}"><code>${esc(occurrence.source_id)}</code> in ${esc(workflow.title)}</button></li>`; }).join(''); details.innerHTML = `<h2>${esc(node.label)}</h2><p><span class="decode-status ${esc(node.state)}">${esc(node.state)}</span> ${esc(node.decision || '')}</p><p><strong>Semantic overview:</strong> ${occurrences.length} visible source occurrence${occurrences.length === 1 ? '' : 's'} represented by one canonical node.</p><p>${esc(node.evidence || '')}</p>${h2kg}${role}<h3>Inspect preserved source occurrences</h3><ul>${list || '<li>No occurrence is visible.</li>'}</ul>${anchor ? `<p>Double-click this node to add one complete connected workflow through the shared anchor.</p>` : ''}`; document.querySelectorAll('[data-inspect-occurrence]').forEach((button) => button.addEventListener('click', () => { viewMode = 'source'; syncViewControls(); renderGraph(); showOccurrenceDetails(button.dataset.inspectOccurrence); })); }
  function showAnchorDetails(anchorId) { const anchor = data.anchorById[anchorId]; if (!anchor) return; const workflows = [...new Set((data.anchor_index[anchorId] || []).map((id) => data.nodeById[id]?.workflow_id).filter(Boolean))].map((id) => data.workflowById[id]); const candidates = rankedConnectedWorkflows(anchorId); const h2kg = anchor.state === 'approved_h2kg' ? `<p><a href="${root.dataset.h2kgExplore}?iri=${encodeURIComponent(anchor.id)}">Inspect this stable H2KG term in H2KG Explore</a></p>` : ''; const role = anchor.canonical_role_iri && anchor.state !== 'approved_h2kg' ? `<p><strong>H2KG role:</strong> <a href="${root.dataset.h2kgExplore}?iri=${encodeURIComponent(anchor.canonical_role_iri)}">${esc(anchor.semantic_role || anchor.canonical_role_iri)}</a></p>` : ''; const cards = candidates.slice(0,8).map((workflow) => `<button class="decode-add-workflow" data-add-workflow="${esc(workflow.id)}">Add full workflow: ${esc(workflow.title)}</button>`).join(''); details.innerHTML = `<h2>${esc(anchor.label)}</h2><p><span class="decode-status ${esc(anchor.state)}">${esc(anchor.state)}</span> ${esc(anchor.decision || '')}</p><p><code>${esc(anchor.id)}</code></p><p>${esc(anchor.evidence)}</p>${h2kg}${role}<h3>Linked workflows (${workflows.length})</h3><ul>${workflows.map((workflow) => `<li>${esc(workflow.title)}</li>`).join('')}</ul>${cards ? `<h3>Expand a connected workflow</h3>${cards}` : '<p>All linked workflows are visible.</p>'}`; document.querySelectorAll('[data-add-workflow]').forEach((button) => button.addEventListener('click', () => { shownWorkflows.add(button.dataset.addWorkflow); viewMode = 'semantic'; syncViewControls(); renderGraph(); showAnchorDetails(anchorId); updateMoreButton(); })); }
  function visibleRelations(id) { return [...cy.$id(id).connectedEdges()].map((edge) => `${edge.data('kind')}: ${edge.data('label') || 'relation'}`).join('; ') || 'none'; }
  function showHover(element, event) { const item = element.data(); if (element.isEdge()) { const count = item.occurrenceCount ? `<p><strong>Preserved source dependencies:</strong> ${item.occurrenceCount}</p>` : ''; hover.innerHTML = `<h3>${esc(item.label || item.kind)}</h3><p><strong>Relation layer:</strong> ${esc(item.kind)}</p><p><code>${esc(item.source)}</code> -> <code>${esc(item.target)}</code></p>${count}`; } else if (item.kind === 'semantic-anchor' || item.kind === 'semantic-unresolved') { const node = data.overviewNodeById[item.id]; hover.innerHTML = `<h3>${esc(node.label)}</h3><p><span class="decode-status ${esc(node.state)}">${esc(node.state)}</span></p><p>${node.occurrence_count} preserved occurrence${node.occurrence_count === 1 ? '' : 's'} represented here. Double-click to expand connected workflows when an anchor is available.</p>`; } else if (item.kind === 'source-anchor') { const anchor = data.anchorById[item.id], workflows = [...new Set((data.anchor_index[item.id] || []).map((id) => data.nodeById[id]?.workflow_id).filter(Boolean))]; hover.innerHTML = `<h3>${esc(anchor.label)}</h3><p><span class="decode-status ${esc(anchor.state)}">${esc(anchor.state)}</span></p><p>${workflows.length} linked workflows. Double-click to add the next complete workflow.</p>`; } else { const node = data.nodeById[item.id]; hover.innerHTML = `<h3>${esc(node.label)}</h3><p><span class="decode-status ${esc(node.mapping_state)}">${esc(node.mapping_state)}</span> ${esc(node.visual_category || 'other')}</p><p><strong>GraphML ID:</strong> <code>${esc(node.source_id)}</code><br><strong>Workflow:</strong> ${esc(data.workflowById[node.workflow_id].title)}<br><strong>Native category:</strong> ${esc(node.native_category || 'not supplied')}</p><p><strong>Visible relations:</strong> ${esc(visibleRelations(node.id))}</p>`; } hover.classList.remove('is-hidden'); moveHover(event); }
  function moveHover(event) { if (!event || hover.classList.contains('is-hidden')) return; hover.style.left = `${Math.min((event.clientX || 0) + 14, window.innerWidth - hover.offsetWidth - 10)}px`; hover.style.top = `${Math.min((event.clientY || 0) + 14, window.innerHeight - hover.offsetHeight - 10)}px`; }
  function hideHover() { hover.classList.add('is-hidden'); }
})();
"""
